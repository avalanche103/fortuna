...
Нужно выложить: код + `data/fortuna.db` + все файлы из `public/uploads/` (сейчас это тысячи фото, порядка 5 ГБ).
...
Появится папка `dist-deploy/code` и архив `dist-deploy/fortuna-hosting-code.zip` — код, шаблоны, статика без фото и файл базы. Админ в этой базе уже `fortuna` (пароль из чата при подготовке).
